...
int main(void)
{
  ...
  Queue2Handle = osMessageQueueNew(4, sizeof(uint8_t), &Queue2_attributes);
...
static void MX_GPIO_Init(void)
{
  ...
  /* LD3 (green, PG13) and LD4 (red, PG14) on-board LEDs used for game feedback */
  ...
  /* Joystick SW (fire button) on PC4: module pulls low when pressed, use internal pull-up */
  GPIO_InitStruct.Pin = GPIO_PIN_4;
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_PULLUP;
  HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);

  /* Joystick VRx on PC3 (ADC1_IN13): must be Analog mode, otherwise the digital
     input buffer stays active and skews/biases the voltage the ADC reads */
...
void StartDefaultTask(void *argument)
{
  /* USER CODE BEGIN 5 */
  int8_t dir;
  uint32_t adcCenter = 128;
  const uint32_t adcDead = 40;
  uint32_t calSum = 0;
  uint32_t calCount = 0;
  GPIO_PinState swPrevState = GPIO_PIN_SET; /* released = HIGH (internal pull-up) */
  uint8_t swStableCount = 0;
  const uint8_t SW_DEBOUNCE_TICKS = 2; /* ~2*20ms = 40ms debounce */

  /* Hiệu chuẩn điểm giữa joystick khi bật nguồn (để cần ở giữa ~0.7s) */
  for (int i = 0; i < 64; i++)
  {
    HAL_ADC_Start(&hadc1);
    if (HAL_ADC_PollForConversion(&hadc1, 50) == HAL_OK)
    {
      calSum += HAL_ADC_GetValue(&hadc1);
      calCount++;
    }
    osDelay(10);
  }
  ...
  for (;;)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      {
        sampleSum += HAL_ADC_GetValue(&hadc1);
        sampleCount++;
      }
    }
    ...
    if (sampleCount > 0U)
    {
      uint32_t adc = sampleSum / sampleCount;

      if (adc + adcDead < adcCenter)
      {
        dir = -1;
      }
      else if (adc > adcCenter + adcDead)
      {
        dir = 1;
      }
      else
      ...
      /* Auto re-center: whenever the reading looks "idle" (inside the dead
         zone), slowly nudge the center point toward the live reading. This
         compensates for joystick modules whose mechanical rest position is
         not perfectly at mid-scale and for slow voltage drift (temperature,
         supply, wiring), which is the usual cause of a tank that keeps
         creeping to one side even though nobody is touching the stick.
         Genuine pushes fall outside the dead zone and therefore never move
         the baseline, so intentional movement is not affected. */
      if (dir == 0)
      {
        int32_t diff = (int32_t)adc - (int32_t)adcCenter;
        adcCenter = (uint32_t)((int32_t)adcCenter + diff / 32);
      }

      osMessageQueuePut(Queue1Handle, &dir, 0, 0);
    }

    /* Joystick SW (fire) button on PC4, active low, debounced */
    GPIO_PinState swNow = HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_4);
    if (swNow == swPrevState)
    {
      if (swStableCount < 255)
      {
        swStableCount++;
      }
    }
    ...
    if (swStableCount >= SW_DEBOUNCE_TICKS && swNow == GPIO_PIN_RESET)
    {
      /* Held-down repeats are harmless: FireBullet() self-throttles via its own cooldown */
      uint8_t firePressed = 1;
      osMessageQueuePut(Queue2Handle, &firePressed, 0, 0);
    }
